...
Foam::patchInteractionDataListHW::patchInteractionDataListHW()
:
    List<patchInteractionDataHW>(),
    patchGroupIDs_()
{}
...
:
    List<patchInteractionDataHW>(dict.lookup("patches")),
    patchGroupIDs_(this->size())
{
    const polyBoundaryMesh& bMesh = mesh.boundaryMesh();
    const wordList allPatchNames = bMesh.names();

    const List<patchInteractionDataHW>& items = *this;
...
Foam::patchInteractionDataListHW::patchInteractionDataListHW
(
    const patchInteractionDataListHW& pidl
)
:
    List<patchInteractionDataHW>(pidl),
...
Foam::label Foam::patchInteractionDataListHW::applyToPatch(const label id) const
{
